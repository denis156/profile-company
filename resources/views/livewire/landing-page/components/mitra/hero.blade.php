<section class="relative min-h-screen flex items-center overflow-hidden bg-gradient-to-b from-base-100 to-base-200">
    {{-- Background Pattern --}}
    <livewire:landing-page.components.bg-pattren />

    <div class="container mx-auto max-w-7xl px-6 lg:px-8 py-24 relative z-10">
        <div class="max-w-4xl mx-auto text-center space-y-8">
            {{-- Title --}}
            <div class="space-y-6" data-aos="fade-up">
                <span class="badge badge-primary badge-lg">Partner Terpercaya</span>
                <h1 class="text-5xl lg:text-6xl font-bold text-base-content">
                    Mitra Kerja
                    <p class="text-primary">PT. Fathiyah Nugraha Group</p>
                </h1>
                <p class="text-xl text-base-content/70 max-w-3xl mx-auto leading-relaxed">
                    Kami bangga bermitra dengan perusahaan-perusahaan terkemuka yang mempercayai layanan dan
                    produk kami
                </p>
            </div>

            {{-- Quick Stats --}}
            <div class="grid grid-cols-3 gap-4 md:gap-6 pt-8" data-aos="fade-up" data-aos-delay="200">
                <div class="space-y-1 md:space-y-2">
                    <div class="text-2xl md:text-4xl font-bold text-primary">15+</div>
                    <div class="text-xs md:text-sm text-base-content/60">Mitra Terpercaya</div>
                </div>
                <div class="space-y-1 md:space-y-2">
                    <div class="text-2xl md:text-4xl font-bold text-primary">3</div>
                    <div class="text-xs md:text-sm text-base-content/60">Kategori Industri</div>
                </div>
                <div class="space-y-1 md:space-y-2">
                    <div class="text-2xl md:text-4xl font-bold text-primary">100%</div>
                    <div class="text-xs md:text-sm text-base-content/60">Kepuasan Mitra</div>
                </div>
            </div>
        </div>
    </div>
</section>
